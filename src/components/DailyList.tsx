        ...
        {daily.map((d) => {
          ...
          const label = isToday
            ...
            : d.time === daily[1]?.time
              ...
              : formatDay(d.time, lang);
          return (
            <li
              key={d.time}
              className="flex items-center gap-3 py-2.5 text-sm"
            >
              <span className={`w-24 shrink-0 ${isToday ? "font-semibold" : ""}`}>
                {label}
              </span>
              <WeatherIcon name={spec.icon} className="h-6 w-6 shrink-0" />
              <span className="hidden w-24 flex-1 text-white/60 sm:block">
...
function tempRange(d: DailyPoint): string {
  const min = Math.round(d.temperature_2m_min);
  const max = Math.round(d.temperature_2m_max);
