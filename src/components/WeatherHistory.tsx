...
const RANGE_MAP: Record<Range, { days: number; i18n: "last30Days" | "last90Days" | "lastYear" }> = {
  "30": { days: 30, i18n: "last30Days" },
  "90": { days: 90, i18n: "last90Days" },
  "365": { days: 365, i18n: "lastYear" },
};
...
export default function WeatherHistory({ loc }: { loc: GeoLocation }) {
  ...
  const { data, loading, error } = useArchive(loc, startDate, endDate, settings);
  ...
  return (
    <section className="rounded-3xl bg-white/10 p-6 backdrop-blur-md ring-1 ring-white/20 shadow-xl">
          ...
          {(Object.keys(RANGE_MAP) as Range[]).map((r) => (
            <button
              ...
              className={`rounded-xl px-3 py-1.5 text-xs font-medium transition ${
                range === r ? "bg-white text-sky-700 shadow" : "text-white/80 hover:bg-white/10"
              }`}
            >
              {translate(lang, RANGE_MAP[r].i18n)}
            </button>
          ))}
        </div>
      </div>
      ...
      {error && !loading && (
        <p className="py-10 text-center text-white/70">{translate(lang, "error")}</p>
      )}

      {!loading && !error && daily.length === 0 && (
        <p className="py-10 text-center text-white/70">{translate(lang, "historyHint")}</p>
      )}
            ...
            {daily.map((d) => {
              ...
              return (
                <li key={d.time} className="flex items-center gap-3 py-2 text-sm">
                  ...
                  <span className="hidden w-24 text-white/60 sm:block">
                    {formatPrecipitation(d.precipitation_sum ?? 0, settings)}
                  </span>
                  <div className="ml-auto flex w-28 items-center justify-end gap-2 tabular-nums">
                    ...
                    <span className="h-1.5 w-8 overflow-hidden rounded-full bg-white/20">
                      <span
                        className="block h-full rounded-full bg-white/70"
                        style={{ width: histRange(d.temperature_2m_min, d.temperature_2m_max) }}
                      />
                    </span>
...
function histRange(min: number, max: number): string {
  const span = Math.max(1, Math.round(max) - Math.round(min));
  return `${Math.min(100, 20 + span * 4)}%`;
}
