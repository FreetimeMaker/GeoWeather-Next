  ...
  return (
    <div ref={boxRef} className="relative w-full max-w-lg">
      <div className="flex gap-2">
        ...
        />
        {busy && (
          <div className="self-center text-white/60">
            ...
            <li key={r.id}>
              ...
              >
                <span>
                  ...
                  <span className="block text-xs text-slate-500">{cityLabel(r)}</span>
                </span>
                <span className="rounded-full bg-sky-500 px-3 py-1 text-xs font-medium text-white">
                  {translate(lang, "addCity")}
                </span>
              </button>
            </li>
          ))}
      ...
      {open && results.length === 0 && !busy && (
        <div className="absolute z-40 mt-2 w-full rounded-2xl bg-white/95 px-4 py-3 text-slate-600 shadow-2xl backdrop-blur-md">
