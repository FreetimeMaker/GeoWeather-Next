  ...
  return (
    <section className="rounded-3xl bg-white/10 p-6 backdrop-blur-md ring-1 ring-white/20 shadow-xl">
      <p className="text-sm uppercase tracking-widest text-white/60">
        {translate(lang, "currentConditions")}
      </p>
      ...
      <div className="mt-2 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-lg text-white/80">
            {translate(lang, "feelsLike")}: {displayTemp(cw.apparent_temperature, settings)}
          </p>
        ...
        <div className="grid grid-cols-2 gap-3 text-center">
          <Metric
            ...
            value={`${cw.relative_humidity_2m}%`}
          />
          <Metric
            label={translate(lang, "precipitation")}
            value={`${cw.precipitation ?? 0} mm`}
          />
          <Metric
            label={translate(lang, "wind")}
            value={formatWind(cw.wind_speed_10m, settings)}
          />
          <Metric
            label={translate(lang, "cloudCover")}
            value={`${cw.cloud_cover}%`}
          />
        </div>
      </div>
    </section>
