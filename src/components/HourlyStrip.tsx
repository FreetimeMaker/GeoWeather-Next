      ...
      <div className="flex gap-2 overflow-x-auto pb-2 -mx-2 px-2">
        {slice.map((h, i) => {
          ...
          return (
            <div
              key={h.time + i}
              className="flex min-w-[68px] flex-col items-center gap-1 rounded-2xl bg-white/5 p-2"
            >
              <span className="text-xs text-white/60">
                {i === 0 ? translate(lang, "now") : formatHour(h.time)}
              </span>
              <WeatherIcon
                name={spec.icon}
                className={`h-7 w-7 ${h.is_day ? "" : "text-white/50"}`}
              />
              <span className="text-sm font-semibold">
                {displayTemp(h.temperature_2m, settings)}
              </span>
